...
if(($_POST) && $core->exist($_POST['action']) && base64_decode($_POST['action'])=== $acao){
	if(isset($_FILES['banner_imagem']) && $_FILES['banner_imagem']['size']>0){
		...
		if(is_bool($saveImage) && $saveImage === true){
			$validateAction = $HP->saveBanner($_POST, $uploadImage->fileName);
			$saveImage = null;
			if($validateAction === true){
				$getID = $HP->findBanner($_POST, $uploadImage->fileName);
				$core->redirectTo($admin->path."?admin=hp-banner-form&a=1&id=".base64_encode($getID[0]));
				die();
			}
		}
		else{
		...
		$validateAction = $HP->saveBanner($_POST, $_POST['imagem_atual']);
		if($validateAction === true){
			$getID = $HP->findBanner($_POST, $_POST['imagem_atual']);
			$core->redirectTo($admin->path."?admin=hp-banner-form&a=1&id=".base64_encode($getID[0]));
			die();
		}
	}
...
if(isset($_GET['id']) && $core->exist($_GET['id']) && base64_decode($_GET['id'])>0){
	$bannerValues = $HP->getBanner(base64_decode($_GET['id']));
}
...
?>
	...
	<div id="page-wrapper">
		<div class="row">
			<div class="col-lg-12">
				<h2 class="page-header">HP <i class="fa fa-fw" aria-hidden="true">&#xf101</i> Banner <i class="fa fa-fw" aria-hidden="true">&#xf101</i> Criar/Atualizar </h2>
			</div>
			<!-- /.col-lg-12 -->
		</div>
						...
						<div class="row">
							<div class="col-lg-12">
								...
								<form role="form" method="POST" action="" enctype="multipart/form-data">
									...
									<?php
										if(isset($bannerValues['image'])){
									?>
									
									<input type="hidden" name="imagem_atual" value="<?php echo $bannerValues['image'];?>">
									<div class="form-group">
										<label>Imagem atual</label>
										<p><img class="img-responsive" src="../img/banner-hp/<?php echo $bannerValues['image'];?>"></p>
									</div>
