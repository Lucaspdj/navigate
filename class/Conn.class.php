...
class Conn extends Mysqli
{
    ...
    private $_logFile = './mysql_errors.log';
    ...
    {
        if (! is_array($data))
            return $this->setError('Error: Can\'t prepare data!');
    ...
    public function Select($table, $where = NULL, $cols = '*', $limit = NULL, $order_by = NULL, $order = "ASC")
    {
        $_where = $this->prepareWhere($where);
        $_limit = (!is_null($limit) ? ' LIMIT ' . $limit : '');
        $_order = (!is_null($order_by) ? ' ORDER BY ' . $order_by ." ". $order : '');
        $_sql = "SELECT {$cols} FROM `{$table}`{$_where}{$_order}{$_limit};";
		//echo "<code>".$_sql."</code>";
    ...
    public function Create($table, $fields, $primaryKey = NULL, $engine = 'MyISAM', $drop = FALSE, $charset = 'utf8', $collate = 'utf8_bin')
    {
        if (is_null($table) || ! is_string($table) || (is_string($table) && $table == ''))
            return $this->setError('Error: First parameter `table name` is invalid for method `Create`!');
        if (! is_array($fields))
            return $this->setError('Error: Second parameter `table fields` is invalid for method `Create`!');
        ...
        foreach ($fields as $field => $values) {
            ...
            if (isset($values['auto']) && $values['auto'])
                $_fields .= 'AUTO_INCREMENT ';
            if (isset($values['null']) && $values['null'])
                $_fields .= 'NULL ';
            else
                $_fields .= 'NOT NULL ';
            if (isset($values['deafult']))
                $_fields .= 'DEFAULT ' . (is_string($values['deafult']) ? '\'' . $values['deafult'] . '\'' : $values['deafult']) . ' ';
            if (isset($values['comment']))
                $_fields .= "COMMENT '{$values['comment']}' ";
        ...
        if ($drop)
            $this->Drop($table); // ("DROP TABLE IF EXISTS `{$table}`;\n");
            ...
            elseif (is_array($value)) {
                if (array_key_exists('NOT', $value)) {
                    ...
                    if (count($value['BETWEEN']) !== 2)
                        return $this->setError('Error: `where` clause BETWEEN data size is invalid!');
